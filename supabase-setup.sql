...
-- ─────────────────────────────────────────────────────────────
-- A–Z rebuild additions: poets, special days, monthly awards
-- ─────────────────────────────────────────────────────────────
...
create table if not exists public.poet_works (
  id uuid primary key default gen_random_uuid(),
  poet_id uuid not null references public.poets(id) on delete cascade,
  title text not null,
  work_type text not null default 'poem' check (work_type in ('poem','recitation','children','song','letter','other')),
  body text not null default '',
  source_url text not null default '',
...
create table if not exists public.special_days (
  ...
  title text not null,
  message text not null,
  calendar_type text not null default 'gregorian' check (calendar_type in ('gregorian','bangla','hijri','custom')),
  month_no integer,
  day_no integer,
...
alter table public.poet_works enable row level security;
alter table public.special_days enable row level security;

drop policy if exists "poets public read" on public.poets;
create policy "poets public read" on public.poets for select using (true);
drop policy if exists "poets staff write" on public.poets;
create policy "poets staff write" on public.poets for all to authenticated using (public.is_staff()) with check (public.is_staff());
drop policy if exists "poet works approved read" on public.poet_works;
create policy "poet works approved read" on public.poet_works for select using (status='approved' or contributor_id=auth.uid() or public.is_staff());
drop policy if exists "poet works own submit" on public.poet_works;
create policy "poet works own submit" on public.poet_works for insert to authenticated with check (contributor_id=auth.uid());
drop policy if exists "poet works staff review" on public.poet_works;
create policy "poet works staff review" on public.poet_works for update to authenticated using (public.is_staff()) with check (public.is_staff());
drop policy if exists "special days public read" on public.special_days;
create policy "special days public read" on public.special_days for select using (active=true);
drop policy if exists "special days staff write" on public.special_days;
create policy "special days staff write" on public.special_days for all to authenticated using (public.is_admin_or_founder()) with check (public.is_admin_or_founder());

create or replace function public.calculate_monthly_editor_award(target_month date default (date_trunc('month', now() at time zone 'Asia/Dhaka')::date - interval '1 month')::date)
returns table(winner_id uuid, score numeric, approved_count bigint)
language plpgsql security definer set search_path=public as $$
declare w uuid; s numeric; c bigint; month_end date;
begin
  if not public.is_founder() then raise exception 'Founder role required'; end if;
  month_end := (target_month + interval '1 month')::date;
  select ec.editor_id,
         sum(case when ec.status='approved' then greatest(ec.accuracy_score,70)/10 else -5 end),
         count(*) filter(where ec.status='approved')
  into w,s,c
  from public.editor_contributions ec
  where ec.created_at >= target_month and ec.created_at < month_end
  group by ec.editor_id
  having count(*) filter(where ec.status='approved') >= 1
  order by sum(case when ec.status='approved' then greatest(ec.accuracy_score,70)/10 else -5 end) desc,
           avg(ec.accuracy_score) filter(where ec.status='approved') desc,
           count(*) filter(where ec.status='approved') desc,
           min(ec.created_at) asc
  limit 1;
  if w is null then return; end if;
  insert into public.editor_awards(month_start,winner_id,score,snapshot,awarded_at,expires_at)
  values(target_month,w,s,jsonb_build_object('approved_count',c,'timezone','Asia/Dhaka'),now(),now()+interval '24 hours')
  on conflict(month_start) do update set winner_id=excluded.winner_id,score=excluded.score,snapshot=excluded.snapshot,awarded_at=excluded.awarded_at,expires_at=excluded.expires_at;
  insert into public.site_pins(kind,target_id,title,url,created_by,starts_at,expires_at,active)
  values('profile',w::text,'মাসের সাহিত্য-সম্পাদক','profile.html?user='||w::text,auth.uid(),now(),now()+interval '24 hours',true);
  insert into public.notifications(user_id,actor_id,type,message,url)
  values(w,auth.uid(),'editor_award','অভিনন্দন! আপনি মাসের সাহিত্য-সম্পাদক নির্বাচিত হয়েছেন।','profile.html?user='||w::text);
  return query select w,s,c;
end;$$;
revoke all on function public.calculate_monthly_editor_award(date) from public;
grant execute on function public.calculate_monthly_editor_award(date) to authenticated;

alter table public.poets replica identity full;
alter table public.poet_works replica identity full;
alter table public.special_days replica identity full;
do $$ begin
  if not exists(select 1 from pg_publication_tables where pubname='supabase_realtime' and schemaname='public' and tablename='poet_works') then alter publication supabase_realtime add table public.poet_works; end if;
  if not exists(select 1 from pg_publication_tables where pubname='supabase_realtime' and schemaname='public' and tablename='special_days') then alter publication supabase_realtime add table public.special_days; end if;
end $$;
