/* BHUBONDANGA MASTER LOCK V7.1.0 — NIGHT SILVER + CANONICAL SOCIAL + ANALYTICS — SESSION + FACEBOOK PROFILE + REACTIONS */
(() => {
  'use strict';
  if (window.BhubondangaMaster?.version === '7.1.0') return;
  ...
  W.BhubondangaMaster=Object.freeze({version:'7.1.0',client,auth,toast,setMessagePermission,refreshPostSocial,react,openStoryDoor,openMagicDoor});
...
/* =====================================================================
   BHUBONDANGA V7.1 — NIGHT SILVER / CANONICAL REACTIONS / COMPOSER STUDIO
   Loaded after the legacy master. This layer owns visual consistency only;
   it does not create duplicate feeds, sidebars, reaction rows or scroll locks.
   ===================================================================== */
(() => {
  'use strict';
  ...
  let raf=0;function scheduleRepair(){if(raf)return;raf=requestAnimationFrame(()=>{raf=0;repairReactionArt();normalizeLiteraryCards()})}
  ...
  /* Capture legacy reaction pickers before their old handlers can resize/rerender/scroll. */
  W.addEventListener('click',e=>{
    const pick=e.target.closest?.('[data-bd31-tray-pick],[data-pick],[data-bd24-pick],[data-bd64-pick]');
    if(pick){const k=keyFrom(pick),card=postCard(pick),id=postId(card);if(!k||!id)return;e.preventDefault();e.stopImmediatePropagation();W.BhubondangaMaster?.react?.(id,k).catch?.(()=>{});return}
    const main=e.target.closest?.('[data-bd31-react-main],[data-bd143-like],[data-bd64-react-main]');
    if(main){const card=postCard(main),id=postId(card),k=keyFrom(main)||String(main.dataset.current||'like');if(!id)return;e.preventDefault();e.stopImmediatePropagation();W.BhubondangaMaster?.react?.(id,rx.includes(k)?k:'like').catch?.(()=>{});return}
  },true);

  /* One double-tap owner at WINDOW capture: prevents old document engines from firing twice. */
  let bd7Tap={card:null,t:0,x:0,y:0};
  W.addEventListener('pointerup',e=>{if(e.isPrimary===false||e.target.closest?.('button,a,input,textarea,select,label,audio'))return;const card=postCard(e.target),body=e.target.closest?.('.post-body,.post-text,.body,.post-media,video');if(!card||!body)return;const now=Date.now(),same=bd7Tap.card===card&&now-bd7Tap.t<330&&Math.hypot(e.clientX-bd7Tap.x,e.clientY-bd7Tap.y)<18;if(!same){bd7Tap={card,t:now,x:e.clientX,y:e.clientY};return}bd7Tap={card:null,t:0,x:0,y:0};const id=postId(card);if(!id)return;e.preventDefault();e.stopImmediatePropagation();W.BhubondangaMaster?.react?.(id,'love').catch?.(()=>{});const burst=D.createElement('span');burst.className='bd-love-burst';burst.innerHTML=art('love');if(getComputedStyle(card).position==='static')card.style.position='relative';card.appendChild(burst);setTimeout(()=>burst.remove(),620)},true);
  ...
  function boot7(){buildStudio();repairReactionArt();normalizeLiteraryCards();installViews();monthlyWinner();deepLink();const mo=new MutationObserver(()=>{scheduleRepair();installViews()});mo.observe(D.body||D.documentElement,{subtree:true,childList:true});}
